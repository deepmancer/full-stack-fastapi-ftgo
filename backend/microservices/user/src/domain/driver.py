...
class DriverDomain(UserDomain):
    async def register_vehicle(self, plate_number: str, license_number: str) -> Optional[Dict[str, Any]]:
        try:
            ...
            logger.error(ErrorCodes.VEHICLE_SUBMISSION_ERROR.value, payload=payload)
    ...
    async def get_vehicle_info(self) -> Optional[Dict[str, str]]:
        try:
            ...
            logger.error(ErrorCodes.VEHICLE_NOT_FOUND_ERROR.value, payload=payload)
    ...
    async def delete_vehicle(self):
        try:
            ...
            payload = {'user_id': self.user_id}
            logger.error(ErrorCodes.VEHICLE_REMOVE_ERROR.value, payload=payload)
